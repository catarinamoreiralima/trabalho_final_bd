/* ============================================================================================================
   ...
   ARQUIVOS DE ENTRADA (diretório: data/):
...
-- - data/ é o caminho local onde os arquivos estão armazenados neste projeto

-- Carregar dados da Fórmula 1 (F1) para staging
\copy stg_circuits              FROM 'data/circuits.csv'                 WITH (FORMAT csv, HEADER true, ENCODING 'LATIN1')
\copy stg_constructors          FROM 'data/constructors.csv'             WITH (FORMAT csv, HEADER true)
\copy stg_drivers               FROM 'data/drivers.csv'                  WITH (FORMAT csv, HEADER true)
\copy stg_races                 FROM 'data/races.csv'                    WITH (FORMAT csv, HEADER true)
\copy stg_driver_standings      (season, round, driver_id, position, points, wins)      FROM 'data/driver_standings.csv'      WITH (FORMAT csv, HEADER true)
\copy stg_constructor_standings (season, round, constructor_id, position, points, wins) FROM 'data/constructor_standings.csv' WITH (FORMAT csv, HEADER true)
\copy stg_results               FROM 'data/results.csv'                  WITH (FORMAT csv, HEADER true)
\copy stg_qualifying            FROM 'data/qualifying.csv'               WITH (FORMAT csv, HEADER true)

-- Carregar dados geográficos complementares para staging
\copy stg_airports              FROM 'data/airports.csv'                 WITH (FORMAT csv, HEADER true)
\copy stg_countries             FROM 'data/countries.csv'                WITH (FORMAT csv, HEADER true)
\copy stg_time_zones            FROM 'data/timeZones.tsv'                WITH (FORMAT csv, HEADER true, DELIMITER E'\t')
\copy stg_feature_codes         FROM 'data/featureCodes_en.tsv'          WITH (FORMAT csv, HEADER false, DELIMITER E'\t')
\copy stg_iso_language_codes    FROM 'data/iso-languagecodes.tsv'        WITH (FORMAT csv, HEADER true, DELIMITER E'\t')
\copy stg_cities                FROM 'data/cities.tsv'                   WITH (FORMAT csv, HEADER false, DELIMITER E'\t')
...
DROP TRIGGER IF EXISTS trg_normalize_city_name ON cities;
...
BEGIN
    ...
    LOOP
        -- Reutilizar o standing se a carga já tiver sido executada antes.
        SELECT st.id
        INTO v_standings_id
        FROM standings st
        JOIN driver_standings ds
          ON ds.standing_id = st.id
        WHERE st.season_id = rec.season_id
          AND st.round = rec.round
          AND st.position = rec.position::INTEGER
          AND st.points = rec.points
          AND st.wins = rec.wins
          AND ds.driver_id = rec.driver_id
        LIMIT 1;

        IF v_standings_id IS NULL THEN
...
BEGIN
    ...
    LOOP
        ...
        JOIN constructor_standings cs
          ON cs.standing_id = st.id
        WHERE st.season_id = rec.season_id
          AND st.round = rec.round
          AND st.position = rec.position::INTEGER
          AND st.points = rec.points
          AND st.wins = rec.wins
          AND cs.constructor_id = rec.constructor_id
        LIMIT 1;
